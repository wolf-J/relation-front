  ...
  <div class="page">
    ...
    <Card dis-hover style="margin-top:10px">
      <Row>
        ...
        <Col span="1">
          <Button type="info" size="large" @click="hdlShowAdjustTaskModel"
            >转件</Button
          >
        </Col>
      </Row>
      ...
      <Modal
        v-model="showAdjustTaskModel"
        title="人工转件"
        ok-text="提交"
        @on-ok="adjustTaskList"
        scrollable
      >
        <Form
          :model="taskObject"
...
import {
  ...
  adjust_task_list_query_url,
  post,
  queryByPage,
  flow_record_url,
  adjust_task_list_url,
  select_users_url
} from "@/api/data";
...
export default {
  ...
  mounted: function() {
    ...
    post(select_users_url)
      ...
      .catch(err => {
        this.$Message.error("获取信审员信息失败：" + JSON.stringify(err));
      });
  },
  computed: {},
  methods: {
    ...
    hdlShowAdjustTaskModel() {
      if (this.flowTaskIdList.length === 0) {
        this.$Message.error("请选择至少一条记录");
      } else {
        this.showAdjustTaskModel = true;
      }
    },
    adjustTaskList() {
